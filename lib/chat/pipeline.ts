    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          toDate: { type: 'string', description: 'End date in YYYY-MM-DD format (optional, defaults to 7 days after fromDate).' },
        },
        required: ['fromDate'],
      },
    },
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          description: { type: 'string', description: 'Brief description of what they want to discuss (1 sentence)' },
          startAt: { type: 'string', description: 'Booking start time as ISO 8601 UTC string (from a check_availability result)' },
        },
...
export async function runChatPipeline(input: PipelineInput): Promise<PipelineOutput> {
  const { message, conversationId, orgSlug, visitorName, visitorEmail } = input;
  ...
  const { data: learnings } = await supabase
    ...
    .eq('org_id', org.id)
    .limit(10);
  const learningContext = learnings?.map((l) => `Q: ${l.question}\nA: ${l.answer}`).join('\n\n') || '';
...
- result "success": "Done — your booking [bookingCode] (scheduled for [localTime]) has been cancelled. A confirmation has been sent to your email."`;
  ...
  const executeTool: ToolExecutor = async (name, args) => {
    ...
    if (name === 'create_booking') {
      const { name: bookingVisitorName, email, description, startAt } = args as Record<string, string>;
      const result = await createBooking({ orgSlug, visitorName: bookingVisitorName, visitorEmail: email, description, startAt });
      ...
      void (async () => {
        try {
          ...
          await Promise.all([
            sendBookingConfirmationEmail({
              toEmail: email,
              visitorName: bookingVisitorName,
    ...
    if (name === 'cancel_booking') {
      ...
      if (!booking) return JSON.stringify({ result: 'not_found' });
      if (booking.visitor_email.toLowerCase() !== email.toLowerCase()) return JSON.stringify({ result: 'email_mismatch' });
      if (booking.status === 'cancelled') return JSON.stringify({ result: 'already_cancelled' });
  ...
  if (response.includes('[ESCALATE:') && visitorName && visitorEmail) {
    const reason = response.match(/\[ESCALATE:\s*(.+?)\]/)?.[1] || 'User requested human assistance';
  ...
  await supabase.from('messages').insert({ conversation_id: convId, role: 'assistant', content: response });
